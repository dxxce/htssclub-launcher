...
interface ShortReelsPlayerProps {
  url: string;
...
export default function ShortReelsPlayer({ url, episodeId, poster, title, subtitles = [], aspectRatio, onEnded, subtitleSize, pip }: ShortReelsPlayerProps) {
    ...
    const initPlayer = async () => {
      try {
        ...
        const playerOptions: any = {
            ...
            m3u8: function (video: HTMLMediaElement, url: string, art: any) {
              ...
              if (Hls.isSupported()) {
                ...
                hls.on(Hls.Events.ERROR, function (event, data) {
                  if (data.fatal) {
                    console.error('Hls.js Fatal Error (ShortReels):', data.type, data.details);
                    switch (data.type) {
                    ...
                    // Suppress harmless warnings that Hls.js seamlessly auto-recovers from
                    if (data.details !== 'fragParsingError' && data.details !== 'bufferSeekOverHole') {
                      console.warn('Hls.js Warning (Non-fatal, ShortReels):', data.type, data.details);
                    }
                  }
                });
          ...
          airplay: true,
          theme: '#8a2be2', // Purplish accent color for Short Reels player
          lang: 'vi',
          ...
          controls: (pip !== undefined && pip) ? [
            {
              ...
              click: async function () {
                ...
                // Create PiP window via Rust Backend
                try {
                  setIsPipActive(true);
  ...
  if (!cleanUrl) {
    return (
      <div className={`w-full ${aspectRatio || 'aspect-video bg-bg-card'} flex items-center justify-center text-text-secondary`}>
  ...
  return (
    <div className={`relative ${aspectRatio || 'w-full aspect-video'} overflow-hidden shadow-glow-lg bg-black`}>
