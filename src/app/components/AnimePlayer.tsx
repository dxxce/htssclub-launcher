...
export default function AnimePlayer({ url, episodeId, poster, title, subtitles = [] }: AnimePlayerProps) {
      ...
      try {
        ...
        if (defaultSub && defaultSub.file) {
          playerOptions.subtitle = {
            url: defaultSub.file.includes(PROXY_BASE_URL) ? defaultSub.file : `${PROXY_BASE_URL}?url=${encodeURIComponent(defaultSub.file)}`,
            type: 'vtt',
            style: {
              color: '#fff',
              fontSize: '24px',
